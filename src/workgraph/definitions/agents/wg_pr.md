...
1. Run `git fetch origin main`, then `git rebase origin/main`. Resolve
   any conflict. Then squash the branch to exactly one commit with
   `git reset --soft $(git merge-base HEAD origin/main)` and
   `git commit`, then `git push --force-with-lease`. Never use the local
   `main`: it is stale. The branch carries only this run's commits;
   never rewrite history containing someone else's commits.
